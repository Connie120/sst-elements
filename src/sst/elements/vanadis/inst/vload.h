...
public:
    ...
    virtual void
    computeLoadAddress_SIMT(SST::Output* output, VanadisRegisterFile* regFile, uint64_t* out_addr, uint16_t* width, 
                            uint64_t wid, uint16_t tid)
    {
        uint64_t mem_addr_reg_val = regFile->getIntReg_SIMT<uint64_t>(phys_int_regs_in[0], wid, tid);
        ...
        output->verbose(
            CALL_INFO, 16, 0, "[execute-load]: (add)            : %" PRIu64 "\n", (mem_addr_reg_val + offset));
#endif
        int64_t tmp_val = regFile->getIntReg_SIMT<int64_t>(phys_int_regs_in[0], wid, tid);
