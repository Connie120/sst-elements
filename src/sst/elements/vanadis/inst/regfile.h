...
public:
    VanadisRegisterFile(
        ...
        const VanadisFPRegisterMode fp_rmode, bool simt) :
        hw_thread(thr),
    ...
    void copyFromRegister_simt(uint16_t reg, uint32_t offset, uint8_t* values, uint32_t len, bool is_fp, uint16_t tid) {
        if(is_fp) {
            copyFromFPRegister_simt(reg, offset, values, len, tid);
        } else {
            copyFromIntRegister_simt(reg, offset, values, len, tid);
        }
    }

    void copyFromFPRegister_simt(uint16_t reg, uint32_t offset, uint8_t* values, uint32_t len, uint16_t tid) {
        assert(reg < count_fp_regs);
        assert((offset + len) <= fp_reg_width);
    ...
    void copyFromIntRegister_simt(uint16_t reg, uint32_t offset, uint8_t* values, uint32_t len, uint16_t tid) {
        assert(reg < count_int_regs);
        assert((offset + len) <= int_reg_width);
    ...
    void copyToRegister_simt(uint16_t reg, uint32_t offset, uint8_t* values, uint32_t len, bool is_fp, uint16_t tid) {
        if(is_fp) {
            copyToFPRegister_simt(reg, offset, values, len, tid);
        } else {
            copyToIntRegister_simt(reg, offset, values, len, tid);
        }
    }

    void copyToIntRegister_simt(uint16_t reg, uint32_t offset, uint8_t* values, uint32_t len, uint16_t tid) {
        assert((offset + len) <= int_reg_width);
        assert(reg < count_int_regs);
    ...
    void copyToFPRegister_simt(uint16_t reg, uint32_t offset, uint8_t* values, uint32_t len, uint16_t tid) {
        assert((offset + len) <= fp_reg_width);
        assert(reg < count_fp_regs);
    ...
    template <typename T>
    T getIntReg_simt(const uint16_t reg, const uint16_t tid)
    {
        assert(reg < count_int_regs);
        assert(sizeof(T) <= int_reg_width);
    ...
    template <typename T>
    T getFPReg_simt(const uint16_t reg, const uint16_t tid)
    {
        assert(reg < count_fp_regs);
        assert(sizeof(T) <= fp_reg_width);
    ...
    template <typename T>
    void setIntReg_simt(const uint16_t reg, const T val, const uint16_t tid, const bool sign_extend = true)
    {
        assert(reg < count_int_regs);
    ...
    template <typename T>
    void setFPReg_simt(const uint16_t reg, const T val, const uint16_t tid)
    {
        assert(reg < count_fp_regs);
        assert(sizeof(T) <= fp_reg_width);
    ...
    void print_simt(SST::Output* output, int level = 8 )
    {
        output->verbose(CALL_INFO, level, 0, "Integer Registers:\n");

        for ( uint16_t i = 0; i < count_int_regs; ++i ) {
            for (uint16_t j = 0; j < WARP_SIZE; ++j) {
                printRegister_simt(output, true, i, level, j);
            }
        }
        ...
        for ( uint16_t i = 0; i < count_fp_regs; ++i ) {
            for (uint16_t j = 0; j < WARP_SIZE; ++j) {
                printRegister_simt(output, false, i, level, j);
            }
        }
    }
...
private:
    ...
    char* getIntReg_simt(const uint16_t reg, const uint16_t tid)
    {
        assert(reg < count_int_regs);
        return int_reg_storage + (int_reg_width * reg * WARP_SIZE + int_reg_width * tid);
    }

    char* getFPReg_simt(const uint16_t reg, const uint16_t tid)
    {
        assert(reg < count_fp_regs);
    ...
    void printRegister_simt(SST::Output* output, bool isInt, uint16_t reg, const uint16_t tid, int level = 8)
    {
        char* ptr = NULL;

        if ( isInt ) { ptr = getIntReg_simt(reg, tid); }
        else {
            ptr = getFPReg_simt(reg, tid);
        }
